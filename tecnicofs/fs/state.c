...
static char freeinode_ts[INODE_TABLE_SIZE]; //byte-map mai facil do q bitmap

/* Data blocks */
static char fs_data[BLOCK_SIZE * DATA_BLOCKS];
static char free_blocks[DATA_BLOCKS]; //byte-map para saber quais ficheiros estão livres

/* Volatile FS state */

static open_file_entry_t open_file_table[MAX_OPEN_FILES];
static char free_open_file_entries[MAX_OPEN_FILES]; //onse se encontra a proxima entrada livre
